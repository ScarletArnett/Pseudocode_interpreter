...
public class IncExpression extends UnaryExpression {
    public IncExpression(Expression expression) {
        super(expression);
    ...
    public Expression evaluate(InterpContext ctx) {
        ...
        if (exp instanceof IntExpression) {
            ...
            IntExpression result = new IntExpression(val.add(BigInteger.ONE));
        ...
        if (exp instanceof FloatExpression) {
            ...
            FloatExpression result = new FloatExpression(val.add(BigDecimal.ONE));
            ctx.put(var.getValue(), result);
            return result;
        }

        throw new NeigeRuntimeException("you cannot increment a %s", exp.getReflectiveName());
    }
